...
def record_attendance(employee_name, status, location_link="", leave_reason=""):
    try:
        ...
        location_data = ""
        if location_link:
            if "Coordinates:" in location_link:
                # Extract coordinates and address
                parts = location_link.split("|")
                coordinates = parts[0].replace("Coordinates:", "").strip()
                address = parts[1].replace("Address:", "").strip() if len(parts) > 1 else ""
                
                # Create Google Maps link
                lat, lng = coordinates.split(",")
                maps_link = f"https://www.google.com/maps?q={lat},{lng}"
                
                location_data = f"{address} | {maps_link}"
            else:
                location_data = location_link
