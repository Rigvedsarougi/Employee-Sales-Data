...
LOCATION_HISTORY_COLUMNS = [
    "Employee Name", "Employee Code", "Designation", "Date", "Time",
...
VISIT_SHEET_COLUMNS = [
    ...
    "Visit Notes", "Visit Selfie Path", "Visit Status", "Remarks"
]
...
TICKET_SHEET_COLUMNS = [
    "Ticket ID", "Raised By (Employee Name)", "Raised By (Employee Code)",
    "Raised By (Designation)", "Raised By (Email)", "Raised By (Phone)",
    "Category", "Subject", "Details", "Status", "Date Raised", "Time Raised",
    "Resolution Notes", "Date Resolved", "Priority"
]

TRAVEL_HOTEL_COLUMNS = [
    "Request ID", "Request Type", "Employee Name", "Employee Code", "Designation",
    "Email", "Phone", "Adhara Number", "Hotel Name", "Check In Date",
    "Check Out Date", "Travel Mode", "From Location", "To Location",
    "Booking Date", "Remarks", "Status", "Date Requested", "Time Requested"
]

DEMO_SHEET_COLUMNS = [
    "Demo ID", "Employee Name", "Employee Code", "Designation", "Partner Employee",
    "Partner Employee Code", "Outlet Name", "Outlet Contact", "Outlet Address",
    "Outlet State", "Outlet City", "Demo Date", "Check-in Time", "Check-out Time",
    "Check-in Date Time", "Duration (minutes)", "Outlet Review", "Remarks",
    "Status", "Products", "Quantities"
]

TICKET_CATEGORIES = [
    "HR Department", "MIS & Back Office", "Digital & Marketing", "Co-founders",
    "Accounts", "Admin Department", "Travel Issue", 
    "Product - Delivery/Quantity/Quality/Missing", "Others"
]
...
# Location History Functions (using original conn)
...
# Attendance Functions (using original conn)
...
# History Data Functions (using history_conn)
def log_sales_to_gsheet(sales_data):
    try:
        existing_sales_data = history_conn.read(worksheet="Sales", ttl=5)
        ...
        history_conn.update(worksheet="Sales", data=updated_sales_data)
...
def update_delivery_status(invoice_number, product_name, new_status):
    try:
        sales_data = history_conn.read(worksheet="Sales", ttl=5)
        ...
        history_conn.update(worksheet="Sales", data=sales_data)
        return True
    except Exception as e:
...
def log_visit_to_gsheet(visit_data):
    try:
        existing_visit_data = history_conn.read(worksheet="Visits", ttl=5)
        ...
        history_conn.update(worksheet="Visits", data=updated_visit_data)
...
def log_ticket_to_gsheet(ticket_data):
    try:
        existing_data = history_conn.read(worksheet="Tickets", usecols=list(range(len(TICKET_SHEET_COLUMNS))), ttl=5)
        existing_data = existing_data.dropna(how="all")
        updated_data = pd.concat([existing_data, ticket_data], ignore_index=True)
        history_conn.update(worksheet="Tickets", data=updated_data)
        return True, None
    except Exception as e:
        return False, str(e)
...
def log_travel_hotel_request(request_data):
    try:
        existing_data = history_conn.read(worksheet="TravelHotelRequests", usecols=list(range(len(TRAVEL_HOTEL_COLUMNS))), ttl=5)
        existing_data = existing_data.dropna(how="all")
        updated_data = pd.concat([existing_data, request_data], ignore_index=True)
        history_conn.update(worksheet="TravelHotelRequests", data=updated_data)
        return True, None
    except Exception as e:
        return False, str(e)
...
def log_demo_to_gsheet(demo_data):
    try:
        existing = history_conn.read(worksheet="Demos", usecols=list(range(len(DEMO_SHEET_COLUMNS))), ttl=5).dropna(how="all")
        df_new = pd.DataFrame([demo_data], columns=DEMO_SHEET_COLUMNS)
        history_conn.update(worksheet="Demos", data=pd.concat([existing, df_new], ignore_index=True))
        return True
    except Exception as e:
            ...
            with st.form(key='delivery_status_form'):
                ...
                if submitted:
                    with st.spinner("Updating delivery status..."):
                        try:
                            all_sales_data = history_conn.read(worksheet="Sales", ttl=5)
                            ...
                            history_conn.update(worksheet="Sales", data=all_sales_data)
...
def visit_page():
    ...
    with tab2:
        ...
        if st.button("Search Visits", key="search_visits_button"):
            try:
                visit_data = history_conn.read(worksheet="Visits", ttl=5)
...
def support_ticket_page():
    ...
    with tab2:
        ...
        try:
            tickets_data = history_conn.read(worksheet="Tickets", usecols=list(range(len(TICKET_SHEET_COLUMNS))), ttl=5)
...
def travel_hotel_page():
    ...
    with tab3:
        ...
        try:
            requests_data = history_conn.read(worksheet="TravelHotelRequests", usecols=list(range(len(TRAVEL_HOTEL_COLUMNS))), ttl=5)
...
def demo_page():
    ...
    with tab2:
        ...
        def load_demo_data():
            try:
                df = history_conn.read(worksheet="Demos", usecols=list(range(len(DEMO_SHEET_COLUMNS))), ttl=5)
